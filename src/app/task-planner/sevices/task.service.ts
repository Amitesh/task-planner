...
export class TaskService {
  ...
  post(taskListId: string, task: Task): Observable<TaskList> {
    return this.http.post<TaskList>(`${this.taskListUrl}\/${taskListId}\/tasks`, task, httpOptions) 
  }

  put(taskListId: string, task: Task): Observable<TaskList> {
    return this.http.put<TaskList>(`${this.taskListUrl}\/${taskListId}\/tasks\/${task._id}`, task, httpOptions) 
  }

  delete(taskListId: string, task: Task): Observable<TaskList> {
    return this.http.delete<TaskList>(`${this.taskListUrl}\/${taskListId}\/tasks\/${task._id}`, httpOptions) 
  }
